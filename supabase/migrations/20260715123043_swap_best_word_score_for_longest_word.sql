-- Harfik — önceki migration "En Uzun Kelime"yi yanlış anlamayla kaldırmıştı;
-- asıl istenen "En Yüksek Kelime Puanı" (best_word_score) idi. Bu migration:
--   1. games.longest_word kolonunu geri ekler,
--   2. games.best_word_score ve games.best_word kolonlarını kaldırır,
--   3. player_stats view'ını buna göre yeniden oluşturur (longest_word geri,
--      best_word_score/best_word kaldırıldı).

drop view if exists public.player_stats;

alter table public.games add column if not exists longest_word text;
alter table public.games drop column if exists best_word_score;
alter table public.games drop column if exists best_word;
