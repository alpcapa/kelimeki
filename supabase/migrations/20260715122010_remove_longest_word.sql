-- Harfik — "En Uzun Kelime" istatistiği anlamsız bulunduğu için kaldırıldı.
-- player_stats view'ı önce drop edilir (games.longest_word'e bağımlı),
-- sonra kolon kaldırılır, sonra view longest_word olmadan yeniden oluşturulur.

drop view if exists public.player_stats;

alter table public.games drop column if exists longest_word;
