...
| Bu PR (10 Eyl, kozmetik) | Arkadaşlar modalının sekme adları: "Arkadaşlarım" → **Arkadaşlar**, "İstekler" → **Davetler** (boş durum: *"Bekleyen davet yok."*) | ⚠ **SÜRÜME BİNİYOR:** `ui/friends/friends_modal.dart` — sekme etiketleri ve boş durum metni web ile BİREBİR (kullanıcı isteği, 10 Eylül 2026). Yalnızca ETİKET: sekme kimlikleri (`FriendsTab.requests`), RPC adları ve veri dili ("istek") değişmedi. `test/friends_test.dart` etiketi büyük harfle arıyor (`DAVETLER`) |
