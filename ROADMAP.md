...
| (11 Eyl, FAZ C 24.5) | Mağaza kareleri **başlıklı** oldu + **7. kare** (k-lig sıralaması) eklendi | `integration_test/store_screenshots_test.dart` (+ `.github/workflows/ios-screenshots.yml`). ⚠ **Sıradaki sürüme HİÇBİR ŞEY taşımıyor** — `integration_test/` mağazaya giden ikiliye girmez, yalnızca `flutter drive` ile koşar ve uygulama kodu hiç değişmedi. Tabloda olmasının sebebi #497/#505'in aynısı: `mobile/app/` altına dokunmuş olması |
