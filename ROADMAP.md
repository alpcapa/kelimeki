...
| (11 Eyl, FAZ C 24.5) | Modal kareleri (04 · 06 · 07) artık **oyun ekranının üstünde** açılıyor + karelerin **yerel önizlemesi** (`npm run preview-store-frames`) | `integration_test/store_frames.dart` (YENİ — ortak sahne kurulumu), `integration_test/store_screenshots_test.dart`, `test/store_frames_preview_test.dart` (YENİ, `KARE_ONIZLEME=1` kapısının arkasında) (+ kök `package.json`). ⚠ **Sıradaki sürüme HİÇBİR ŞEY taşımıyor** — `integration_test/` ve `test/` mağazaya giden ikiliye girmez, uygulama kodu (`lib/`) hiç değişmedi. Tabloda olmasının sebebi #497/#505'in aynısı: `mobile/app/` altına dokunmuş olması. Kullanıcı isteği: *"önce resim olarak yap bana göster ondan sonra ok ise üretime gönderelim"* — kayıt `marketing/app-store/console-formlari.md` §13 |
