...
| **24.3** APNs / push | ✅ `GoogleService-Info.plist` · `Runner.entitlements` · `AppDelegate` bildirim kanalı · ✅ **cihaz yarısı KANITLANDI** (15 Eyl: `push_tokens`te iOS satırları var, 2 kişi). ⛔ **AMA BİLDİRİM DÜŞMÜYOR — APNs anahtarı `RL4JLXL389` SANDBOX-ONLY** ve TestFlight token'ı production; her gönderim `403 BadEnvironmentKeyInToken` (aşağı, 24.3 bölümü). Kalan iş konsolda: anahtarın ortamını *Sandbox & Production* yap (Edit izin vermezse yeni anahtar + Firebase'e yükle) — **istemcide değişecek satır YOK, yeni sürüm GEREKMİYOR** |
