...
| **1.1.0** | **656** (iOS) | `c7ac2e9` | 11 Eyl 2026 (`c7ac2e9`, #528) | **— GÖNDERİLMEDİ** (11 Eyl 23:02'de ASC History'den ölçüldü: gönderim satırı YOK) | ⏳ **Prepare for Submission** — metadata giriliyor | İki Canlı oyun düzeltmesi (sahte "Sıra sende değil." + sonsuz "Yükleniyor…") + mağaza kareleri. ⚠ **Bu satır 11 Eyl akşamı DÜZELTİLDİ:** önce *"654 · 16:08 · App Store incelemesine GÖNDERİLDİ · ⏳ incelemede"* diyordu. Gerçek: `Add for Review` basıldı ama ikinci adım (Review Submission → `Submit to App Review`) tamamlanmadı; Description/Keywords o saatte BOŞTU, yani gönderim zaten geçemezdi. İliştirilen build 629 → 654 → **656**; 656'nın kodu 654 ile birebir aynı (aradaki fark yalnızca doküman/CSV), numara Play ile hizalı dursun diye seçildi. ⚠ Play tarafı hâlâ **627**'de ve `mobile-latest`teki `.aab` artık **656 DEĞİL 659** (`7bccbf7`, koşu #659, 11 Eyl 21:04Z — `mobile-build.yml`in `paths` düzeltmesi kendi derlemesini tetikledi ve 656'nın paketini ÜZERİNE YAZDI; `mobile-latest` her mobil derlemede ezilir). **Kayıp yok:** 659'un uygulama kodu 656 ile BİREBİR AYNI — `git diff c7ac2e9 7bccbf7` = workflow + doküman/CSV, `mobile/app/lib` altında tek satır yok. Play'e gidecek paket bu yüzden **1.1.0 (659)**. ⚠ Bedeli numara hizası: ASC'de iliştirilen build **656**, yani 11 Eylül akşamı 654→656 kaydırmasının gerekçesi (iki mağaza tek numarada dursun) düştü — hizayı isteyen ASC'de 659'u iliştirir, kod aynı olduğundan başka hiçbir şey değişmez. Aşağı bkz. |
