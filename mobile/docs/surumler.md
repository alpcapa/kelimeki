...
| **1.1.1** | **723** | `8c1828f` | 25 Eyl 2026 (tur sonu PR'ı) | **App Store: 25 Eyl 13:21 incelemeye GÖNDERİLDİ, 25 Eyl 22:49 ONAYLANDI** (`Pending Developer Release`, elle yayın — Play'le aynı gün) · Play: ~27-28 Eyl (kullanıcı kararı) | ⏳ Apple onaylı, yayın Play'i bekliyor | Merge turunun on PR'ı (#565 · #562 · #579 · #576 · #554 · #557 · #547 · #601 · #611 · #626); ayrıntı aşağıda |
...
Yani App Store bir sürümü ONAYLADIKTAN sonra o numaraya yeni build almıyor;
`+N` build numarası yetmiyor. 710 · 714 · 717 `cancel-in-progress` ile
iptal oldu. Kullanıcı kararı (25 Eyl): **1.1.1** (düzeltme/ince ayar
turu). `pubspec.yaml` + `config/env.dart` birlikte (`app_version_parity_test`).

⚠ **Ders:** bir sürüm App Store'da onaylandıktan sonra `main`'e giren ilk
mobil iş SÜRÜM NUMARASINI da yükseltmeli, yoksa her `main` derlemesi
...
- [x] **ASC: 1.1.1 kaydı + build 723 + incelemeye GÖNDERİLDİ** (25 Eyl
      13:21 TSİ, `Waiting for Review`, Submission ID `6df19844-…`). Kullanıcı
      kararı: Apple Android'i beklemeden gider; yayın **Manually release**,
      yani onay gelince Play'le AYNI GÜN elle yayınlanır. Notes 1.1.0'dan
      aynen kaldı (§16'nın kalıcı cevabı). Android turunda hata çıkarsa:
      `Cancel Submission` → yeni build → yeniden gönder.
- [ ] Play'e `.aab` (**723**, ~27-28 Eyl) — AYNI paket ("SÜRÜM SENKRONU").
...
- [x] **Apple onayı geldi: 25 Eyl 22:49 TSİ** (e-posta "Review of your
      ...
      Durum `Pending Developer Release` — BİLEREK henüz yayınlanmadı.
- [ ] `Release This Version` → **Play'deki 1.1.1 (723) yayına girdiği
      gün**, önce değil. Önkoşul: `.apk` turu A–E ✅. Turda hata çıkarsa
      onaylı ama yayınlanmamış sürüm "Developer Reject" ile geri çekilip
      yeni build'le yeniden gönderilebilir; yayınlanmış olan geri ALINAMAZ.

## 1.1.0 (665) — ✅ İKİ MAĞAZADA YAYINDA (Play production 24 Eyl 2026)
...
- [x] **ASC** → 1.1.0 sürüm kaydındaki build 659 → **665** ✅ **YAPILDI**
      (12 Eyl, kullanıcı bildirdi). **Aynı gün 17:32'de sürüm İNCELEMEYE
      GÖNDERİLDİ:** App Privacy (Published) + Pricing (Free) girildi,
      metadata zaten 11 Eyl'de girilmişti; durum
      `Prepare for Submission` → **`Waiting for Review`**. Yayın **elle**
      (Manually release), yani Apple onaylasa bile mağazaya kendiliğinden
      DÜŞMEZ — Play ile aynı gün yayınlanabilir. Ayrıntı:
...
- [x] **Play** → ✅ **YAYINDA (24 Eyl 2026, 17:44).** Gönderim **#19**,
      production, `1.1.0 (665)` · *Start full rollout*, gönderildi 13 Eyl
      00:49 (kullanıcının Console ekran görüntüsünden okundu). Senkron
      penceresi KAPANDI: iOS ve Play aynı paket, `9c62289`. **Merge
      dondurması kalktı** — kullanıcı kararı: tur 25 Eyl'de başlar, Play'e
      yeni paket hemen gönderilmez (önce canlıdaki 665 izlenir).
      Eski not: `mobile-latest`teki `.aab` (665) yüklenecek (13 Eyl).
...
- [x] İkisi de bitti (24 Eyl) → bölüm YAYINDA; 659 pasif.
