...
- **Sekmeler (`AdminDashboard.tsx`):**
  ...
  - **Oyunlar** — `admin_game_counts` RPC'si Toplam/2/4 Kişilik için başlatılan vs. biten oyun sayısını verir (`games` tablosu — yalnızca girişli kullanıcıların skor kayıtları, `game_starts`'la karşılaştırılır).
...
- **Büyüme > Oyun** — `admin_game_activity_series(p_periods, p_granularity, p_scope, p_player_count)`: aynı granülerlik + Toplam/Kayıtlı/Misafir (`p_scope`, `user_id is/is not null`'a göre) + Toplam/2/4 Kişilik (`p_player_count`) filtreleriyle başlatılan (`game_starts`) / tamamlanan (`game_finishes` where `completed=true`) / terk edilen (`completed=false`) oyun sayıları ve ortalama süre döner (genel + aynı oturum/çok oturumlu kırılımı, yalnızca tamamlanan oyunlardan hesaplanır — terk edilenlerin "süresi" gerçek oynama süresini değil ne kadar sonra fark edilip silindiğini yansıttığından ortalamaya katılmaz). `game_starts`/`game_finishes` tamamen anonim sayaç tablolarıdır (skor/kelime gibi kişisel veri yok) — misafir oyunlar için de `user_id=null` satır alır (`game_starts_insert_anon`/`game_finishes_insert_anon` RLS politikaları), bu yüzden `games`'ten (yalnızca girişli) farklı olarak gerçek toplam kullanım hacmini yansıtır. `GrowthChart.tsx` tek bir generic bileşen; tıklanabilir legend'larla seri açıp kapatma, tablo görünümü ve (süre grafiğinde) `formatDuration` ile sn/dk/sa/gün/hafta/ay/yıl kademeli biçimlendirme sağlar.
