# ROADMAP Arşivi — Cilt 1 (DONDURULDU: 27 Ağustos – 12 Eylül 2026)

> **Dondurulmuş cilt — buraya YAZILMAZ.** Yeni kapanan maddeler aktif cilde,
> `docs/decisions/roadmap-arsiv.md`'ye taşınır. `npm run check-doc-size` bu
> dosyayı `FROZEN` listesinde tutar; büyürse CI düşer ("yanlışlıkla arşive
> yazdım" hatasının tek yakalayıcısı).
>
> **Neden ayrıldı (25 Eylül 2026):** aktif arşiv 258 KB'a gelmişti, `reference`
> uyarısına (260 KB) ~2 KB kalmıştı ve 24 Eylül'de başlayan merge turu
> (#565 → … → #626) yeni kapanan maddeler taşıyacaktı. Başlığın önerdiği aday
> kesme ("sürüm turları ↔ kapanmış maddeler") yerine **İçindekiler
> tablosunun altı** kesildi: dosya zaten iki katmandı — üstte 15-24 Eylül
> taşımaları (madde 31, FAZ C, güvenlik #19-#20, iPad manzara), altta 2
> Eylül'deki ilk taşımanın gövdesi + 1.0.7-1.1.0 sürüm turları + madde 23
> fazları. Alt katman artık hiç değişmiyor; dondurulacak olan o. Hiçbir satır
> değiştirilmedi, başlıklar ve madde numaraları aynen korundu; içindekiler
> tablosu aktif ciltte kaldı.
>
> **Atıf bulma:** koddaki ve dokümanlardaki `roadmap-arsiv.md → "X"` atıfının
> karşılığı aktif ciltte yoksa buradadır — `grep -n "X" docs/decisions/roadmap-arsiv*.md`.
...
## 1.1.0 sürüm turu — ✅ **SAHADA** (11-12 Eylül 2026)

⚠ **Bu tur İKİ pakete bölündü ve ikisinin de adı 1.1.0** — kütükte ayrı
satırlar: **627** (`a4c809b`, 11 Eyl 08:01 gönderim, ≤ 08:33 yayın) ve
**659** (`7bccbf7`, 12 Eyl ≤ 12:24 gönderim, ~13:42 yayın). Aradaki TEK
ikili farkı #525 (Canlı oyun: sahte *"Sıra sende değil."* + sonsuz
*"Yükleniyor…"*); kalan altı commit `integration_test/` ya da dev
bağımlılığı.

⚠ **Turun asıl yeniliği paket değil KAPSAM:** aynı derleme (659) hem Play'in
kapalı testine hem App Store Connect'in 1.1.0 sürüm kaydına gitti — iki
mağaza ilk kez tek NUMARADA ve tek PAKETTE. App Store tarafının kendi
kütüğü `marketing/app-store/console-formlari.md`'de.

Paket künyeleri, sürüm notları, gönderim/yayın saatleri ve yayın sonrası
kontrol listesi: `mobile/docs/surumler.md` → "1.1.0 (659)" ve "1.1.0 (627)".

Aşağıdaki tablo `ROADMAP.md` → "Sıradaki sürüme binecekler"den OLDUĞU GİBİ
taşındı (12 Eylül 2026, 659 yayınlanınca — dosyanın kendi kuralı: ROADMAP
yalnızca AÇIK maddeleri tutar). Satırlar değişmedi.

**1.0.9'dan (581, `1abde38`) beri porta dokunan işler — 1.1.0'IN İKİ
PAKETİNİN içeriği** (627 ve 659):
...
| (11 Eyl, FAZ C 24.5) | Modal kareleri (04 · 06 · 07) artık **oyun ekranının üstünde** açılıyor + karelerin **yerel önizlemesi** (`npm run preview-store-frames`) | `integration_test/store_frames.dart` (YENİ — ortak sahne kurulumu), `integration_test/store_screenshots_test.dart`, `test/store_frames_preview_test.dart` (YENİ, `KARE_ONIZLEME=1` kapısının arkasında) (+ kök `package.json`). ⚠ **Sıradaki sürüme DAVRANIŞ taşımıyor** — `integration_test/` ve `test/` mağazaya giden ikiliye girmez. ⚠ **DÜZELTME (11 Eyl gece): `lib/` DEĞİŞTİ**, bu satır önce *"uygulama kodu (`lib/`) hiç değişmedi"* diyordu ve bu YANLIŞTI — `8708896` `ui/setup/setup_screen.dart`e `showDiagnostics` bayrağını ekledi (varsayılan `true`, `false` veren TEK çağıran `integration_test/store_frames.dart`). Sonuç değişmiyor (üretim davranışı aynı), ama gerekçe ölçülerek yazılmalıydı: `git diff --stat <paket-sha>..origin/main -- mobile/app/lib`. Tabloda olmasının sebebi #497/#505'in aynısı: `mobile/app/` altına dokunmuş olması. Kullanıcı isteği: *"önce resim olarak yap bana göster ondan sonra ok ise üretime gönderelim"* — kayıt `marketing/app-store/console-formlari.md` §13 |
| (11 Eyl) | **Sonsuz "Yükleniyor…"** — yavaş/asılı bağlantıda Canlı oyun ekranı açılmıyordu | ⚠ **SÜRÜME BİNİYOR:** `data/online_games_api.dart` — ilk yükleme (`loadGame`) zaten var olan 20 sn'lik `_callTimeout`un altına alındı. `try/catch` yalnızca REDDEDİLEN isteği yakalar; hiç bitmeyeni yakalayan tek şey zaman aşımı. Tavan iki tarafta da VARDI ama yalnızca arka plan çağrılarındaydı. Web ikizi AYNI PR'da (`src/components/OnlineGameScreen.tsx` → `withTimeout`). Kapı `fakeAsync` ile (gerçek zamanla 20 sn sürerdi), duyarlılığı kanıtlandı; **844 test yeşil**. Kayıt: `docs/decisions/live-game.md` + Parça 202 |
| (11 Eyl) | **Sahte "Sıra sende değil."** — hamle sunucuda işlenmişken ekranda hata çıkıyordu; ayrıca ham `PostgrestException` dökümü kullanıcıya gösteriliyordu | ⚠ **SÜRÜME BİNİYOR:** `data/online_api.dart` (yeni `ServerRejection` — sunucu reddi mesajına soyuluyor), `data/online_games_api.dart` (zincirin üç halkasına `moveId` eklendi), `ui/live/online_game_screen.dart` (idempotency anahtarı HAMLEYE bağlı tutuluyor, başarıda temizleniyor — play/pass/exchange). Kullanıcı iPhone'da bildirdi, canlıdan doğrulandı (hamlenin TEK satırı vardı). Web ikizi AYNI PR'da: `src/lib/api.ts` + `src/components/OnlineGameScreen.tsx` artık `p_move_id` gönderiyor (ROADMAP'te açık borçtu). Üç kapı, üçü de duyarlılığı kanıtlanarak; **843 test yeşil**. Kayıt: `docs/decisions/live-game.md` + Parça 201 |
